...
// .wrangler/tmp/pages-3IbOgp/functionsWorker-0.9699526782601772.mjs
...
var PLAN_LIMITS = { free: 10, pro: 200, agency: 1e3 };
var PLAN_PRICES = {
...
async function ensureSchema(db) {
  await db.exec(`
    CREATE TABLE IF NOT EXISTS users (
      id TEXT PRIMARY KEY, api_key TEXT UNIQUE NOT NULL, plan TEXT NOT NULL DEFAULT 'free',
      subscription_status TEXT DEFAULT 'active', daily_lead_count INTEGER DEFAULT 0,
      daily_message_count INTEGER DEFAULT 0, total_lead_count INTEGER DEFAULT 0,
      usage_date TEXT DEFAULT '', created_at TEXT NOT NULL DEFAULT (datetime('now'))
    );
    CREATE TABLE IF NOT EXISTS leads (
      id TEXT PRIMARY KEY, user_id TEXT NOT NULL, business_name TEXT NOT NULL,
      industry TEXT, location TEXT, contact_channel TEXT, contact_info TEXT,
      lead_score INTEGER DEFAULT 50, buying_intent_reason TEXT, outreach_strategy TEXT,
      outreach_email TEXT, outreach_instagram TEXT, outreach_linkedin TEXT,
      best_channel TEXT, lead_category TEXT DEFAULT 'warm', region_competition_score INTEGER DEFAULT 50,
      source TEXT DEFAULT 'ai_generated', status TEXT DEFAULT 'new',
...
function leadRow(row) {
  return {
    ...
    buying_intent_reason: row.buying_intent_reason,
    outreach_strategy: row.outreach_strategy,
    outreach_email: row.outreach_email,
    outreach_instagram: row.outreach_instagram,
    outreach_linkedin: row.outreach_linkedin,
    best_channel: row.best_channel,
    lead_category: row.lead_category,
    region_competition_score: row.region_competition_score,
    source: row.source,
    status: row.status,
...
function estimateRevenue(score, industry) {
  const multi = { SaaS: 1.5, Fintech: 2, Healthcare: 1.8, "E-commerce": 0.8, Manufacturing: 1, Education: 0.6, Technology: 1.3 };
  const m = multi[industry] || 1;
  const base = 500 + score * 20;
  const est = Math.round(base * m);
  return { low: Math.round(est * 0.5), high: Math.round(est * 2), estimated: est, currency: "USD" };
}
__name(estimateRevenue, "estimateRevenue");
__name2(estimateRevenue, "estimateRevenue");
async function callDeepSeek(env, messages, maxTokens = 6e3, temp = 0.6) {
...
async function generateLeads(env, keyword, industry, location, limit) {
...
[{"business_name":"NexaGrid","industry":"Cloud SaaS","location":"Austin TX","contact_channel":"linkedin","contact_info":"linkedin.com/in/contact","lead_score":82,"buying_intent_reason":"Recently raised Series A","outreach_strategy":"Connect on LinkedIn","lead_category":"hot","region_competition_score":65}]`;
  try {
    ...
    return mockLeads(keyword, industry, location, limit);
  }
}
__name(generateLeads, "generateLeads");
__name2(generateLeads, "generateLeads");
async function validateLead(env, lead) {
...
Return ONLY JSON: {"is_valid": true|false, "adjusted_score": 0-100, "confidence": 0.0-1.0, "rejection_reason": ""}`;
  try {
    ...
    return { is_valid: true, adjusted_score: lead.lead_score || 50, confidence: 0.7, rejection_reason: "" };
  }
}
__name(validateLead, "validateLead");
__name2(validateLead, "validateLead");
async function generateOutreachBatch(env, leads) {
  const leadsText = leads.map((l, i) => `Lead ${i + 1}: ${l.business_name} \u2014 ${l.industry} \u2014 Contact: ${l.contact_info} \u2014 Channel: ${l.contact_channel}`).join("\n\n");
...
[{"lead_index":1,...}, {"lead_index":2,...}]`;
  try {
    ...
    return leads.map((l) => mockOutreach(l));
  }
}
__name(generateOutreachBatch, "generateOutreachBatch");
__name2(generateOutreachBatch, "generateOutreachBatch");
async function generateSuggestion(env, lead) {
...
Output ONLY the strategy text. No JSON, no markdown.`;
  try {
    return await callDeepSeek(env, [{ role: "user", content: prompt }], 256, 0.5);
  } catch {
    const s = lead.lead_score || 50;
    return `Follow up within 48 hours. Score ${s}/100 \u2014 ${s > 70 ? "high priority" : "standard priority"}.`;
  }
}
__name(generateSuggestion, "generateSuggestion");
__name2(generateSuggestion, "generateSuggestion");
function calculateFinalScore(initial, validation) {
...
__name2(calculateFinalScore, "calculateFinalScore");
function mockLeads(keyword, industry, location, limit) {
...
__name(mockLeads, "mockLeads");
__name2(mockLeads, "mockLeads");
function mockOutreach(lead) {
...
__name(mockOutreach, "mockOutreach");
__name2(mockOutreach, "mockOutreach");
async function checkUsage(env, apiKey, action = "lead") {
  const db = env.DB;
...
app.post("/api/leads/generate", async (c) => {
  ...
  const leadsData = await generateLeads(c.env, keyword, industry, location, limit);
  const outreachResults = await generateOutreachBatch(c.env, leadsData);
  const db = c.env.DB;
  const results = [];
  let rejected = 0, totalRevLow = 0, totalRevHigh = 0, highIntent = 0;
  for (let i = 0; i < leadsData.length; i++) {
    const ld = leadsData[i];
    const validation = await validateLead(c.env, ld);
    ...
    await db.prepare(`INSERT INTO leads (id, user_id, business_name, industry, location, contact_channel, contact_info, lead_score, buying_intent_reason, outreach_strategy, outreach_email, outreach_instagram, outreach_linkedin, best_channel, lead_category, region_competition_score, created_at, updated_at)
      ...
      ld.contact_channel || outreach.best_channel || "email",
      ld.contact_info || "",
      finalScore,
      ld.buying_intent_reason || "",
      ld.outreach_strategy || "",
      outreach.email || "",
      outreach.instagram || "",
      outreach.linkedin || "",
      outreach.best_channel || "email",
      category,
      ld.region_competition_score || 50,
      now,
      now
    ).run();
    totalRevLow += revenue.low;
    totalRevHigh += revenue.high;
    if (finalScore >= 70) highIntent++;
    results.push({
      id: leadId,
      business_name: ld.business_name,
      industry: ld.industry || industry,
      location: ld.location || location,
      contact_channel: ld.contact_channel || outreach.best_channel || "email",
      contact_info: ld.contact_info || "",
      lead_score: finalScore,
      buying_intent_reason: ld.buying_intent_reason || "",
      outreach_strategy: ld.outreach_strategy || "",
      outreach_email: outreach.email || "",
      outreach_instagram: outreach.instagram || "",
      outreach_linkedin: outreach.linkedin || "",
      best_channel: outreach.best_channel || "email",
      lead_category: category,
      region_competition_score: ld.region_competition_score || 50,
      source: "ai_generated",
      status: "new",
      created_at: now,
      updated_at: now,
  ...
  return c.json({
    ...
    kpi: {
      ...
      estimated_revenue_low: totalRevLow,
      estimated_revenue_high: totalRevHigh,
...
app.get("/api/leads", async (c) => {
  ...
  if (category) {
    query += " AND lead_category = ?";
    params.push(category);
  }
  const { total } = await db.prepare(`SELECT COUNT(*) as total FROM (${query})`).bind(...params).first() || { total: 0 };
...
app.get("/api/leads/kpi", async (c) => {
  ...
  const user = await db.prepare("SELECT * FROM users WHERE api_key = ?").bind(apiKey).first();
  if (!user) return c.json({ detail: "Invalid API key" }, 404);
  const leads = (await db.prepare("SELECT * FROM leads WHERE user_id = ?").bind(user.id).all()).results;
  ...
  return c.json({
    ...
    high_intent_leads: highIntent.length,
    estimated_revenue_potential: totalRev,
    conversion_probability: Math.round(highIntent.length / Math.max(leads.length, 1) * 100 * 10) / 10,
    plan: user.plan,
    limit: PLAN_LIMITS[user.plan] || 10
  });
});
...
app.post("/api/leads/export", async (c) => {
  ...
  const user = await db.prepare("SELECT * FROM users WHERE api_key = ?").bind(apiKey).first();
  if (!user) return c.json({ detail: "Invalid API key" }, 404);
  const leads = (await db.prepare("SELECT * FROM leads WHERE user_id = ? ORDER BY lead_score DESC").bind(user.id).all()).results;
...
app.get("/api/plans", (c) => {
  ...
  for (const p of ["free", "pro", "agency"]) {
    const sym = currency === "usd" ? "$" : currency === "eur" ? "\u20AC" : "\xA3";
...
app.get("/api/usage/:api_key", async (c) => {
  ...
  const { total } = await db.prepare("SELECT COUNT(*) as total FROM leads WHERE user_id = ?").bind(user.id).first() || { total: 0 };
  const limit = PLAN_LIMITS[user.plan] || 10;
...
app.get("/api/usage/:api_key/logs", async (c) => {
  ...
  const user = await db.prepare("SELECT * FROM users WHERE api_key = ?").bind(apiKey).first();
  if (!user) return c.json({ detail: "User not found" }, 404);
  const leads = (await db.prepare("SELECT * FROM leads WHERE user_id = ? ORDER BY created_at DESC LIMIT ?").bind(user.id, limit).all()).results;
...
var EXPERIMENTS = {
  ...
  cta_copy: { variants: ["start_free_trial", "get_customers_now", "see_leads"], traffic_split: [0.5, 0.3, 0.2] },
...
function assignVariant(experiment, userId) {
  ...
  const hash = Array.from(new TextEncoder().encode(`${experiment}:${userId}`)).reduce((h, b) => h * 31 + b & 4294967295, 0);
...
__name(assignVariant, "assignVariant");
__name2(assignVariant, "assignVariant");
...
// .wrangler/tmp/bundle-Nj4i8B/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Nj4i8B/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9699526782601772.js.map
